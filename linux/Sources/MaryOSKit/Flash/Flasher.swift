...
public struct Flasher: Sendable {
    ...
    public func verify(target: DiskInfo, imageSize: Int64) async throws -> DiskInfo {
        guard DiskUtil.isValidDiskIdentifier(target.bsdName) else {
            throw MaryOSError("\(target.bsdName) is not a whole-disk identifier")
        }
        let fresh = try await diskUtil.info(target.bsdName)
        if let reason = DiskFilter.rejectionReason(fresh) {
            throw MaryOSError("Refusing to write to \(fresh.bsdName) (\(fresh.displayName)): \(reason)")
        }
        guard fresh.totalSize == target.totalSize else {
            throw MaryOSError("\(fresh.bsdName) changed size since it was selected (\(fresh.totalSize) vs \(target.totalSize)); not writing")
        }
        guard imageSize <= fresh.totalSize else {
            throw MaryOSError("Image (\(ByteCountFormatter.string(fromByteCount: imageSize, countStyle: .file))) is larger than \(fresh.bsdName) (\(fresh.sizeDescription))")
        }
        return fresh
    }
    ...
    public func flash(
        ...
        let workDir = fileManager.temporaryDirectory.appending(path: "MaryOS/\(UUID().uuidString)")
        ...
        guard result.succeeded, sawEnd else {
            ...
            throw MaryOSError("Writing the card failed: \(detail)")
        }
        step(.eject, .done)
