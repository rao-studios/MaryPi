...
- **Wi-Fi has never joined a network.** The desktop image carries iwd, with
  networkd giving the link its address, and System Settings › Network scans and
  joins through `iwctl`; the VM has no Wi-Fi card, so none of it has run. The
  server image still has no Wi-Fi configuration.
