...
| System apps | Calculator (Spotlight only): a keypad with precedence, percent, memory, repeated equals and the keyboard. Preview (pinned, and the Finder's viewer for pictures and PDFs): fit, zoom steps, quarter turns, pages and the folder's other pictures; the desktop gained event sources and routing by file kind for the apps to come, and the builder and image carry their libraries (chapter 9) |
