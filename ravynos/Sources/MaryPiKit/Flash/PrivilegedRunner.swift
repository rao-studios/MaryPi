...
public struct PrivilegedRunner: Sendable {
    ...
    public func run(script: URL, prompt: String = "MaryPi needs administrator rights to write the Raspberry Pi card.") async throws -> CommandResult {
        ...
        if !result.succeeded, result.stderrText.contains(Self.userCancelledCode) {
            throw MaryPiError("Administrator authorization was cancelled; the card was not touched.")
        }
        return result
    }
}
