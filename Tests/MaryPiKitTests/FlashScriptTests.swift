...
@Suite struct FlashScriptTests {
    ...
    @Test func rejectsNonWholeDisk() {
        #expect(throws: MaryPiError.self) {
    ...
    @Test func parsesDDProgress() {
        ...
        #expect(Flasher.bytesTransferred(in: "MARYPI:STEP write") == nil)
    }

    @Test func parsesHdiutilAttach() {
        #expect(ImageBuilder.parseAttachedDisk("/dev/disk6          \t                               \t\n") == "disk6")
        #expect(ImageBuilder.parseAttachedDisk("/dev/disk12\n/dev/disk12s1\n") == "disk12")
        #expect(ImageBuilder.parseAttachedDisk("hdiutil: attach failed") == nil)
    }
}
